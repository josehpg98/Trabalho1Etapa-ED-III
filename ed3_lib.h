...
// Ler uma lista de n�meros de um arquivo e preenxer o vetor...
...
/**
 *Preenxe um vetor lendo os n�meros de um arquivo.
 *Os n�meros devem estar um por linha.
 **/
bool fill_vector_from_file(const std::string &filename, std::vector<int32> &vect)
{
  vect.resize(0);
  ...
  if (!ifs.is_open())
  {
    std::cout << "Erro ao ler o arquivo " << filename << ". O arquivo provavelmente n�o existe." << std::endl;
    return false;
  }
  while (!ifs.eof())
  {
...
FuncTimer::~FuncTimer()
{
  ...
  std::cout << "A fun��o " << this->func_name << " levou " << (end - start) << " mcs para ser conclu�da." << std::endl;
}
#endif
