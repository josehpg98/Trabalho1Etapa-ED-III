...
int main()
{
    ...
    try {
        ...
        while(sair==false)
        {
            cout<<"Escolha uma op��o de ordena��o:"<<endl;
            ...
            switch(x)
            {
                case 1:
                {
                    reset_vector(logs);
                    counting_sort(logs);
                    cout<<"O culpado �: "<<logs[index]->user<<endl;
...
void radix_sort(vector<shared_log>& logs)
{
    ...
    //Primeiro, vamos classificar os elementos por m�s.
    for(uint32 i=0; i<logs.size(); i++)
    {
    //Converte o m�s em um index para ser usado.
        int index=getIndex(logs[i]->month);
        months[index].push_back(logs[i]);
    }
    logs.clear();

    //Segundo passo, percorrer os 12 vetores dos m�ses e classificar cada um deles.
    for(uint32 i=0; i<months.size(); i++)
    {
        ...
        //Para o radix funcionar, precisamos descobrir quem � o maior elemento.
        ...
        //Terceiro passo, vamos iterar sobre as casas decimais para podermos ordenar por d�gitos...
        for(uint32 place=1; (higher/place)>0; place*=10)
        {
            //Nossa matriz de classifica��o  por d�gitos vai aqui...
            vector<vector<shared_log>> digits;
            digits.resize(10);

            //Agora percorremos o m�s indicado por I e vamos ordenando d�gito a d�gito...
            for(uint32 i1=0; i1<months[i].size(); i1++)
            {
                //Vamos calcular o nosso d�gito que nos dar� um n�mero entre 0 e 9.
                int32 digit=(months[i][i1]->id/place)%10;
                digits[digit].push_back(months[i][i1]);
            }

            //E remontamos nosso vetor do m�s...
            months[i].clear();
...
void counting_sort(vector<shared_log>& logs)
{
    ...
    //Percorrer os m�ses e ordenar cada vetor...
    for(uint32 i=0; i<months.size(); i++)
    {
        if(months[i].size()==0)
        {
